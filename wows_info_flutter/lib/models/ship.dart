...
class Ship {
  ...
  late String name;
  late String imgLink;
  late double avgDamage;
  late double avgFrags;
  late double avgWinRate;
  ...
  Ship(this.id, this.tier, this.type, this.name, this.nation, this.imgLink,
      this.isSpecial, this.isPremium);

  factory Ship.fromMap(
      Map<String, dynamic> ship, int id, Map<String, dynamic> expMap) {
    Ship buildShip = Ship(
      id,
      ship['tier'],
      ship['type'],
    ...
    buildShip.avgDamage = (expMap['average_damage_dealt'] as num).toDouble();
    buildShip.avgFrags = (expMap['average_frags'] as num).toDouble();
    buildShip.avgWinRate = (expMap['win_rate'] as num).toDouble();
    return buildShip;
  }

  ShipStats buildStats(Map shipJson) {
    ShipStats stats =
        ShipStats(id, tier, type, name, nation, imgLink, isSpecial, isPremium);

    stats.avgDamage = avgDamage;
    stats.avgFrags = avgFrags;
    stats.avgWinRate = avgWinRate;

    return stats;
  }
}

class ShipStats extends Ship {
  // 非战斗数据
  ...
  ShipStats(super.id, super.tier, super.type, super.name, super.nation,
      super.imgLink, super.isSpecial, super.isPremium);
}
